...
if(WIN32)
	set(JCE2CPP "${GOPATH}/bin/jce2go.exe")
	set(TARS_PATH "${GOPATH}/bin/jce2go.exe")
else()
	set(JCE2CPP "${GOPATH}/bin/jce2go")
	set(TARS_PATH "${GOPATH}/bin/jce2go")
endif()

set(TAF_WEB_HOST "" CACHE STRING "set web host")
IF (TAF_WEB_HOST STREQUAL "")
	set(TAF_WEB_HOST "http://web.tars.com")
ENDIF ()

set(TAF_TOKEN "" CACHE STRING "set web token")
...
function(gen_tars TARGET)
	...
	file(GLOB_RECURSE TARS_INPUT *.jce)
	...
	if (TARS_INPUT)
		...
		foreach(TARS_FILE ${TARS_INPUT})
			...
			add_custom_command(
					...
					COMMAND ${JCE2CPP} -outdir taf-protocol "${TARS_TOOL_FLAG}" "${TARS_FILE}"
					COMMENT "${JCE2CPP} -outdir taf-protocol ${TARS_TOOL_FLAG} ${TARS_FILE}"
					DEPENDS ${JCE2CPP} ${TARS_FILE}
			)
...
macro(gen_server APP TARGET)
	...
	if(TARS_INPUT)
		gen_tars(jce-${TARGET})
		add_dependencies(${TARGET} jce-${TARGET})
	endif()
	...
	IF(WIN32)
		FILE(WRITE ${RUN_UPLOAD_COMMAND_FILE} "EXECUTE_PROCESS(COMMAND ${CMAKE_COMMAND} -E echo ${TAF_WEB_HOST}/api/upload_and_publish -Fsuse=@${TARGET}.tgz -Fapplication=${APP} -Fmodule_name=${TARGET} -Fcomment=developer-auto-upload)\n")
		FILE(APPEND ${RUN_UPLOAD_COMMAND_FILE} "EXECUTE_PROCESS(COMMAND ${TARS_PATH}/thirdparty/bin/curl.exe ${TAF_WEB_HOST}/api/upload_and_publish?ticket=${TAF_TOKEN} -Fsuse=@${TARGET}.tgz -Fapplication=${APP} -Fmodule_name=${TARGET} -Fcomment=developer-auto-upload)\n")
		FILE(APPEND ${RUN_UPLOAD_COMMAND_FILE} "EXECUTE_PROCESS(COMMAND -E echo \n---------------------------------------------------------------------------)\n")
	ELSE()
		FILE(WRITE ${RUN_UPLOAD_COMMAND_FILE} "EXECUTE_PROCESS(COMMAND ${CMAKE_COMMAND} -E echo ${TAF_WEB_HOST}/api/upload_and_publish -Fsuse=@${TARGET}.tgz -Fapplication=${APP} -Fmodule_name=${TARGET} -Fcomment=developer-auto-upload)\n")
		FILE(APPEND ${RUN_UPLOAD_COMMAND_FILE} "EXECUTE_PROCESS(COMMAND curl ${TAF_WEB_HOST}/api/upload_and_publish?ticket=${TAF_TOKEN} -Fsuse=@${TARGET}.tgz -Fapplication=${APP} -Fmodule_name=${TARGET} -Fcomment=developer-auto-upload)\n")
...
message("TAF_WEB_HOST:             ${TAF_WEB_HOST}")
message("TAF_TOKEN:                ${TAF_TOKEN}")
message("-------------------------------------------------------------------------------------")
